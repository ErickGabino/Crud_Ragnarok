		...
		`
			<tr>
				...
				<td class="d-flex justify-content-center flex-column"">
					...
					<button class="btn btn-warning editGod" class="editGod" id="editGod" type="button"><span><i class="fa-solid fa-pen"></i><p class="hidden-md">Edit God</p></span></button>
					<button class="btn btn-info seeGod" class="seeGod" id="seeGod" type="button"><span><i class="fa-solid fa-eye"></i><p class="hidden-md">See God</p></span></button>
				</td>
			</tr>
		`;
	});
...
on(document, "click", "#addGod", (e) => {
	...
	formAddGod.addEventListener("submit", async function (e) {
		...
		let nameGodRepresentation = document.getElementById('nameGodRepresentation').value;
...
on(document, "click", "#removeGiant", (e) => {
	...
	alertify.confirm(
	  ...
	  function () {
		fetch('http://localhost:3800/api/giant/'+id, {method:'DELETE'})
		.then(res => res.json())
		.then(() => location.reload())
		alertify.success("Eliminado")
	  },
